...
class ContactImplicitDirectTranscription(MathematicalProgram):
    ...
    def __add_decision_varibles(self):
        ...
        self.h = self.NewContinuousVariables(rows=1, cols=self.num_time_samples-1, name='h')
        # Add state variables to the program
        nX = self.plant.num_positions() + self.plant.num_velocities()
        self.x = self.NewContinuousVariables(rows=nX, cols=self.num_time_samples, name='x')
        # Add control variables to the program
        nU = self.plant.num_actuators()
        self.u = self.NewContinuousVariables(rows=nU, cols=self.num_time_samples, name='u')
        # Add reaction force variables to the program
        Jn, Jt = self.GetContactJacobian()
        self.numN = Jn.shape[0]
        self.numT = Jt.shape[0]
        self.l = self.NewContinuousVariables(rows=2*self.numN+self.numT, cols=self.num_time_samples, name='l')
        ...
        qhigh = self.plant.GetPositionUpperLimits()
    ...
    def __add_dynamic_constraints(self):
        ...
        for n in range(0, self.num_time_steps-2):
            ...
            self.AddBoundingBoxConstraint(self.minimum_timestep, self.maximum_timestep, self.h[n])
            # Add dynamics as constraints 
            self.AddConstraint(self.__backward_dynamics, 
            ...
            self.AddConstraint(self.__normal_distance_constraint, 
            ...
            self.AddConstraint(self.__sliding_velocity_constraint,
            ...
            self.AddConstraint(self.__friction_cone_constraint, 
                        lb=np.zeros(shape=(3*self.numN,)),
                        ub=np.concatenate([np.full((2*self.numN,), np.inf), np.zeros((self.numN,))],axis=0),
                        vars=np.concatenate((self.x[:,n+1], self.l[:,n+1]), axis=0),
                        description="friction_cone")
            # Add joint limit constraints
            self.AddConstraint(self.__joint_limit_constraint,
                        lb=0,
                        ub=0,
    ...
    def __backward_dynamics(self, z):  
        ...
        dv = (v2 - v1)/h
        # Update the context
        self.plant.SetPositionsAndVelocities(self.context, q2, v2)
        ...
        dq2 = self.plant.MapVelocityToQDot(self.context, v2)
        fq = q2 - q1 - h*dq2
    ...
    def __sliding_velocity_constraint(self, z):
        ...
        q, v = np.split(x, 2)
        self.plant.SetPositionsAndVelocities(self.context, q, v)
        dq = self.plant.MapVelocityToQDot(self.context, v)
        # Get the contact Jacobian
        _, Jt = self.GetContactJacobian(q)
    ...
    def __friction_cone_constraint(self, z):
        ...
        mu = self.get_friction_coefficients(x)
        mu = np.diag(mu)
    ...
    def __get_normal_distance(self,x):
        pass

    def __get_friction_coefficient(self,x):
        pass
