...
def plot_CC(horizontal_position, control, t, iteration, sigmas):
    # nominal trajectory
    x = np.loadtxt('x.txt')
    u = np.loadtxt('u.txt')
    l = np.loadtxt('l.txt')
    ...
    # Plot the horizontal trajectory
    fig1, axs1 = plt.subplots(2,1)
    axs1[0].plot(t, x[0,:], label = 'Reference', linewidth=1.5)
    axs1[1].plot(t, u[:], linewidth=1.5)
    
    for i in range(iteration):
        axs1[0].plot(t, horizontal_position[i,:], label = '$\sigma$ = {f}'.format(f = sigmas[i]), linewidth=1.5)
        axs1[1].plot(t, control[i,:], linewidth=1.5)

    axs1[0].set_title('Chance Constraint Variation W/ Fixted ERM variance = 0.1')
    axs1[0].set_ylabel('Position')
    axs1[0].legend()
